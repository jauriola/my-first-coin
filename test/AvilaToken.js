...
contract("AvilaToken", function(accounts){
  ...
  it('initializes the contract with the correct values', function(){
    return AvilaToken.deployed().then(function(instance){
      ...
      assert.equal(name, 'Avila Token', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol){
      assert.equal(symbol, 'AVL', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard){
      assert.equal(standard, 'Avila Token v1.0', 'has the correct standard');
    });
  });
  it('allocates the total supply upon deplyment', function(){
    return AvilaToken.deployed().then(function(instance){
      tokenInstance = instance;
  ...
  it('transfers token ownership', function(){
    return AvilaToken.deployed().then(function(instance){
      ...
      assert.equal(balance.toNumber(), 750000, 'deducts ammount from the sending account');
    });
  });
  ...
  it('approves token for delegated transfer', function(){
    return AvilaToken.deployed().then(function(instance){
      ...
      assert.equal(allowance.toNumber(), 100, 'stores the allowance for delegetd transfer');
    });
  });
  ...
  it('handles delegated transfer', function(){
    return AvilaToken.deployed().then(function(instance){
      ...
      assert(error.message.indexOf('revert')>= 0, 'cannot transfer value larger than approve ammount');
      ...
      assert.equal(balance.toNumber(), 10, 'adds the ammount from the receiving account');
